...
<template>
  <div
    ...
    class="wrapper"
    :style="`--result-clr:var(--color-${classEN});`"
  >
    ...
    <template v-else>
      ...
      <p class="output">
        ...
        >
        <span v-else>សូមបញ្ចូលលេខទូរសព្ទដែលមានយ៉ាងតិច 6 ខ្ទង់</span>
      </p>
    </template>

    <!-- <p class="footer">
      ទិន្នន័យ: <a :href="dataUrl" download>តារាងទិន្នន័យ</a>
    </p> -->
    <p>
      ក្បួនទស្សន៍ទាយ:
      <a
...
.phone-input {
  & input {
    @apply border border-(--result-clr) p-2 rounded-lg text-center;

    &:focus-visible {
      @apply outline-2 outline-(--result-clr);
    }
  }
}
</style>
